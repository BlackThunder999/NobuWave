const NobuWave = (() => {
    ...
    const showAdminPanel = () => {
        const overlay=document.createElement('div'); overlay.className='modal-overlay';
        overlay.innerHTML=`<div class="modal-card" style="max-height:85vh;overflow-y:auto"><h3>🛡️ Админ-панель</h3>
            ...
            <button class="modal-btn secondary" id="closeAdminBtn" style="margin-top:12px">Закрыть</button></div>`;
        document.body.appendChild(overlay);
        ...
        document.getElementById('banUserBtn').addEventListener('click',async()=>{const u=document.getElementById('banUsername').value.trim();const m=parseInt(document.getElementById('banDuration').value);const r=document.getElementById('banReason').value.trim()||'нарушение правил';if(!u)return;const{data:user}=await supabase.from('users').select('id').eq('username',u).single();await supabase.from('bans').upsert({user_id:user?.id,username:u,reason:r,expires_at:new Date(Date.now()+m*60000).toISOString()});alert(`${u} заблокирован на ${m} минут`);loadBanList();});
