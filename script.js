const NobuChirp = (() => {
    ...
    const setupComposer = () => {
        ...
        newPostBtn.addEventListener('click', async function handler() {
            ...
            try {
                ...
                hashtags.forEach(async (tag) => {
                    ...
                    if (exist) await supabase.from('trends').update({ count: exist.count + 1 }).eq('id', exist.id);
                    else await supabase.from('trends').insert({ hashtag: tag });
                });
